...
//	printf("process_exec error!\n");
  ...
  //printf("sema_down-load\n");
  //printf("thread '%s'  create!\n",token);
  if (tid == TID_ERROR)
...
start_process (void *file_name_)
{
 ...
 // cur->pid = success ? (pid_t)(cur->tid) : PID_ERROR;
  if(success) {
	cur->isLoaded = true;
	//printf("sema_up-load\n");
  }
...
static bool load_segment (struct file *file, off_t ofs, uint8_t *upage,
                          ...
                          bool writable);
static void stacking(char** argv, int argNum, void **esp);
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /* counting arguments */
  tok_ptr = strtok_r(copied_file_name, " ", &next_ptr);
  while (tok_ptr != NULL) {
	  argNum++;
	  tok_ptr = strtok_r(NULL, " ", &next_ptr);
  }

  /* allocate and storing argv */
...
static void stacking(char** argv, int argNum, void **esp) {
	...
	int args_length = 0;
	int word_align;
	addrs = (char **)malloc(sizeof(char*) * argNum);
	...
	for (i = argNum - 1; i >= 0; i--) {
		...
		*esp -= len;
		args_length += len;
		addrs[i] = (char *)(uint32_t *)*esp;
		strlcpy(*esp, argv[i], strlen(argv[i]) + 1);
	}
	
	/* word align & insert NULL */
	word_align = 4 - args_length % 4;
	*esp -= word_align + 4;
	memset(*esp, '\0', word_align + 4);

	/* push args adresses into stack */
	for (i = argNum - 1; i >= 0; i--) {
		*esp -= sizeof(char*);
